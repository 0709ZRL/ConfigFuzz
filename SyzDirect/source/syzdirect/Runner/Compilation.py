...
def PrepareSourceCode(doCheckout = False):
    success = True

    for datapoint in Config.datapoints:
        caseIdx=datapoint['idx']
        kernel_commit=datapoint.get('kernel commit')
        caseSrcDir=Config.getSrcDirByCase(caseIdx)
        # 由于使用shutil.copytree，因此目标目录不能存在
        # 每一个bug case对应一个目录，存着对应的Linux源码
        if not os.path.exists(caseSrcDir):
            if Config.LinuxSrcTemplate != None:
                shutil.copytree(Config.LinuxSrcTemplate,caseSrcDir)
            else:
                Config.logging.info(f"[case {caseIdx}] Start cloning Linux source code due to no Linux template provided")
...
exec $CLANG "$@"
    ...
    # 将上面的脚本写入文件${WorkDir}/emit-llvm.sh中
