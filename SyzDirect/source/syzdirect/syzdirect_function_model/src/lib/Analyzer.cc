...
// 需要把内核各个源码的bitcode按照原有的格式存到一个独立的目录里
...
int main(int argc, char **argv) {
  ...
  for (const auto& p : filesystem::recursive_directory_iterator(std::string(kernelBCDir))) {
	...
	// 加载模块
  // 模块（Modules）是表示代码集合的单位。一个Module可以看成是一个完整的翻译单元（如一个文件），其中包含了多个函数，全局变量，类型定义等。
	OP << "Total " << InputFilenames.size() << " file(s)\n";

	for (unsigned i = 0; i < InputFilenames.size(); ++i) {

    // LLVMContext是LLVM中用于管理全局状态和信息的对象。它包含了LLVM类型系统、常量池、元数据等全局组件。
    ...
    // 将智能指针管理的模块对象转换为普通指针，以便程序后续可以直接控制该模块的生命周期。这通常用于模块加入全局上下文或执行后续操作。
		Module *Module = M.release();
    // 创建模块文件路径的副本并将其包装为StringRef对象，方便操作和存储。这样可以确保在后续操作中，该字符串路径不会因原路径的销毁或修改而丢失。
		StringRef MName = StringRef(strdup(InputFilenames[i].data()));
		GlobalCtx.Modules.push_back(make_pair(Module, MName));
    // 在全局上下文中记录每个模块对应的原始文件路径。这便于后续分析中可以快速定位模块来源文件，支持溯源分析和调试。
  ...
  // 从一个配置文件中读取数据，并将其解析为一个映射（map），其中键是函数名（string），值是该函数对应的参数映射（vector<int>）。
  ...
  // fop其实是file_operations的缩写，是Linux内核中的一个结构体，用于描述文件操作的一系列函数指针。
  // 通过分析内核源码，我们可以发现，内核中的很多系统调用都是通过fop来实现的。
  // 通过分析fop，我们可以找到系统调用的实现函数。
  ...
  // 生成系统调用描述
  // 示例：[8字节:syscall名长度][syscall名][8字节:约束数量]
  // [8字节:约束类型0][8字节:参数索引][8字节:常量值]
  // [8字节:约束类型1][8字节:参数索引][8字节:字符串长度][字符串内容]
  // [8字节:约束类型2][8字节:参数索引][8字节:设备名长度][设备名]
  // [8字节:约束类型3][8字节:参数索引][8字节:设备名长度][设备名][family][type][protocol]
  ...
  // 5、输出所有设备，文件系统，网络接口及其系统调用handler信息（似乎仅仅输出而已）
  ...
  // 6、收集每个系统调用的候选handler建立映射，即一个系统调用可能不只有一个handler对应，那么剩下的就是候选
  ...
  // 7、生成系统调用的签名(constraintDebug, kernel_signature_full, kernel_signature_with_info_full)
