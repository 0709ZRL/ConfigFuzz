...
ConstraintInfo constraintExtraction(CallTraceInfo &callTraceInfo) {
	// 分析一条从系统调用到目标基本块的路径（callTrace），提取路径上的各种约束信息，填充到 ConstraintInfo 结构体中。
	...
	// 找到这条路上的所有switch-case常量约束，如果有就加入到constraintInfo.switchConstSet中
	...
	// 找到这条路上的所有if常量约束，如果有就加入到constraintInfo.ifNotConstSet中
...
vector<TargetSignature> collectSignature(BasicBlock* targetBlock, int index) {
	...
	// 收集能抵达目标块的所有路径，存到这个callTraceInfoList里
	getCallTrace(targetBlock, callTraceInfoList);
	// 按路径长度和间接调用的数量对路径进行排序
...
map<string, BasicBlock*> NewParseDataset(){
				...
				if(CallInst* CI= dyn_cast<CallInst>(&inst)){
					...
					if(getCalledFuncName(CI)=="kcov_mark_block"){
						if(ConstantInt *CCI = dyn_cast<ConstantInt>(CI->getOperand(0))){
							...
							// 一个case出现在了多个不同的函数里，那属于错误，记录下后面报错
	...
	if(DuplicateCases.size()>0){
		raw_fd_ostream DuplicatePoints(StringRef("./duplicate_points.txt"),OutErrorInfo, sys::fs::CD_CreateAlways); // 将重复case写到这个文件里并停止程序，报错，防止后面出错
...
int main(int argc, char **argv) {
	...
	cl::ParseCommandLineOptions(argc, argv, "ConfigFuzz: Target point analysis based on configuration options\n");
	SMDiagnostic Err;
	...
	loadSyscallFile();
	raw_fd_ostream shrinkOutput(StringRef("./syscall_shrink_configfuzz.txt"), OutErrorInfo, sys::fs::CD_CreateAlways);


	ConstraintPass CtPass(&GlobalCtx);
	CtPass.run(GlobalCtx.Modules);

	raw_fd_ostream targetFunctionInfoFile(StringRef("./target_functions_info_configfuzz.txt"), OutErrorInfo, sys::fs::CD_CreateAlways);
	...
	raw_fd_ostream OutputJ(StringRef("./CompactOutput_configfuzz.json"), OutErrorInfo, sys::fs::CD_CreateAlways);
 	...
 	struct OutputFrame{
		...
		struct tcallinfoFrame{
			string tcall; // 系统调用名
			int rank; // 该系统调用的排名（路径排名？）越小越优先
			struct constraintFrame{ // 该系统调用的约束信息（似乎只是值约束）
					struct fuconstraintFrame{ // 常量约束
							string name; // 常量名
							uint64_t value; // 约束值
							fuconstraintFrame(string _name,uint64_t _value):name(_name),value(_value){} // 构造函数
					};
					vector<struct fuconstraintFrame> flag_union_constraints; // 存储常量约束的向量
					string string_constraint; // 字符串约束（如系统调用名，模块名等）
			}constraint;
		};
	...
	for (auto item: Dataset) {
		string caseIdx = item.first; // case id
		struct OutputFrame currentOF;
		currentOF.caseIdx = item.first;

		BasicBlock *targetBB = item.second; // 目标点位的基本块
		Function *targetFunc = targetBB->getParent(); // 目标点位的基本块所属的函数
		...
		// 这个collectSignature函数收集从系统调用到该目标基本块的所有可能路径（call trace），收集所有路径上的约束信息（如switch，if，handler等）
		...
		for (const auto& sig: targetSignatures) {
			...
			// 查看当前的这条路径上是否有switch, handler, if等条件约束
			...
			// 补全系统调用的变体，如ioctl补全为ioctl$FS_IOC_GETFLAGS
			// from bottom to top
